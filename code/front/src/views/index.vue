  ...
  <div class="app-container home">
    ...
    <el-row :gutter="20">
      <el-col :sm="24" :lg="12" style="padding-left: 20px">
        <h2>技术支持</h2>
        <p>
          <b>姓名：</b> <span>{{ contact.supportName }}</span>
        </p>
        <p>
          <b>微信：</b> <span>{{ contact.supportWechat }}</span>
        </p>
        <p>
          <b>联系方式：</b> <span>{{ contact.supportPhone }}</span>
        </p>
         <!-- <p>
        ...
        <p>
          <el-button
            ...
            plain
            @click="goTarget(contact.testUrl)"
            >测试地址</el-button
          >
          <!--
          <el-button
      ...
      <el-col :sm="24" :lg="12" style="padding-left: 50px">
        <el-row>
          <el-col :span="12">
            <h2>系统管理员</h2>
            <p>
              <b>姓名：</b> <span>{{ contact.adminName }}</span>
            </p>
            <p>
              <b>微信：</b> <span>{{ contact.adminWechat }}</span>
            </p>
            <p>
              <b>联系方式：</b> <span>{{ contact.adminPhone }}</span>
            </p>
          </el-col>
        </el-row>
        <!-- <el-row>
    ...
    <!-- 修改对话框 -->
    <el-dialog v-dialogDrag :title="title" :visible.sync="open" width="550px" 
      append-to-body 
      :show-close="false" 
      :close-on-click-modal="false"
      :close-on-press-escape="false">
      <resetPwd @change="changeStatus"/>
    </el-dialog>
  </div>
...
import { getContactInfo } from "@/api/system/config";
  ...
  methods: {
    ...
    getContact(){
      getContactInfo().then(response => {
          ...
          if(!this.contact.testUrl.startsWith('http')){
            this.contact.testUrl = 'http://' + this.contact.testUrl;
          }
        }
      );
    },
    ...
    checkResetPassword(){
      ...
      if(username != 'admin' && decrypt(password)=='Dfyj123'){
        MessageBox.confirm('您的密码还是初始密码，请修改密码！', '系统提示', { 
    ...
    changeStatus(value){
      ...
      if(!value){
        ...
        Cookies.set("password", encrypt(value), { expires: 30 });
      }
    }
  }
};
