    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleDetail"
          v-hasPermi="['work:taskClaim:query']"
        >详情</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          <el-collapse-item title="申报明细" name="3">
            ...
            <el-table :data="form.distributionList" border v-if="form.humanCompanyRelation == 2">
              ...
              <el-table-column label="工时" align="center" prop="workingHours">
                <template slot-scope="scope">
                  <el-input-number v-model="scope.row.workingHours" placeholder="请输入工时" :min="0" :max="maxThreshold" :precision="1" :step="0.1" controls-position="right" @change="salaryHoursChange(scope.$index, scope.row.workingHours)"></el-input-number>
              </template>
              </el-table-column>

              <el-table-column label="工时单价（元/小时）" align="center" prop="workingPrice">
                <template slot-scope="scope">
                  <span>{{ !scope.row.workingPrice ? 0.0 : scope.row.workingPrice.toFixed(1) }}</span>
                </template>
              </el-table-column>
              <el-table-column label="小计（元）" align="center" prop="subTotal">
                <template slot-scope="scope">
                  <span>{{ !scope.row.subTotal ? 0.0 : scope.row.subTotal.toFixed(1) }}</span>
                </template>
              </el-table-column>
...
<script>
import { listDevPageTask, getTask, delTask, distribute, listAppNames, listStaffNames, listProjectNames, 
...
export default {
  ...
  methods: {
    ...
    selectSalaryChange(staffId){
      ...
      // 查询工时单价
      let workingPrice = 0.0;
      let staffArray = this.salaryStaffOptions.filter(item=>item.id == staffId);
      if(!!staffArray && staffArray.length>0){
        workingPrice = staffArray[0].workingPrice;
      }

      this.form.distributionList.forEach(item=>{
        if(item.staffId == staffId){
          item.workingPrice = workingPrice;
          item.subTotal = item.workingPrice*item.workingHours;
        }
      })
    ...
    /** 领薪工时数调整 */
    salaryHoursChange(index,value){
      // 计算领薪小计
      this.form.distributionList[index].subTotal = this.form.distributionList[index].workingPrice*value;
    },
